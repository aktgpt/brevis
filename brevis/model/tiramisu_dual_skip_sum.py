...
class DenseUNetDualSkipSum(BaseModel):
    """DensUNet
    ...
    def __init__(
        ...
        if include_top:
            ...
            #     ),
            #     nn.Conv2d(
            #         in_channels=channels_count + 1 * 2,
            #         out_channels=nb_seg_classes,
            #         kernel_size=1,
            #         stride=1,
            #         padding=0,
    ...
    def forward(self, x):
        ...
        # skip_connections_copy = copy.deepcopy(skip_connections)
        for i in range(0, len(self.up_blocks)):
            skip = skip_connections[-i-1]
            if i == 0:
                x1 = self.transitions_up[i](x, skip, use_skip=True)
            else:
                decoder_sum_connections.append(x1)
                x1 = self.transitions_up[i](x1, skip, use_skip=True)
