...
class LipidNucSegDataset(Dataset):
    def __init__(self, config, csv_file, augment=True):
        ...
        self.p = config["p"]
        if self.safe_crop:
            self.coord_folder = os.path.join(config["folder"], 'masks_lipids_centroids', 
    ...
    def __getitem__(self, idx):
        ...
        target_path = os.path.join(
            self.folder, 'masks_lipids', mag_path, 
        ...
        if self.normalize:
            ...
            image = normalize_image(
                    image, self.fluorecscent_min[channel_name_to_idx('C2')], self.fluorecscent_max[channel_name_to_idx('C2')]
                )
        else:
            image = image / 65536
