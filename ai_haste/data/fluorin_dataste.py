...
class FluorInDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for c in other_channels:
            image.append(cv2.imread(os.path.join(self.folder, mag_path, self.data.iloc[[idx]][c].item()), -1))
        ...
        if self.normalize:
            ...
            for i,c in enumerate(other_channels):
                channel_idx = channel_name_to_idx(c)
                image[:,:,i] = normalize_image(
                    image[:,:,i],
            ...
            if self.standardize:
                ...
                for i,c in enumerate(other_channels):
                    channel_idx = channel_name_to_idx(self.output_channel)
                    image[:,:,i] = (image[:,:,i] - self.fluorecscent_means[channel_idx]
