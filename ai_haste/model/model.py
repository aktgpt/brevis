    ...
    def get_param_count(self):
        ...
        for param in self.parameters():
            if param.requires_grad:
                param_count_grd += np.prod(param.size())
            else:
                param_count_nogrd += np.prod(param.size())
        return param_count_grd, param_count_nogrd

    def summary(self, half=False):
        """ Logs some information about the neural network.
        """
        layers_count = len(list(self.modules()))
        logging.info(f"Model {self} has {layers_count} layers.")
        param_grd, param_nogrd = self.get_param_count()
        param_total = param_grd + param_nogrd
        logging.info(f"-> Total number of parameters: {param_total:n}")
        logging.info(f"-> Trainable parameters:       {param_grd:n}")
        logging.info(f"-> Non-trainable parameters:   {param_nogrd:n}")
        approx_size = param_total * (2.0 if half else 4.0) * 10e-7
        logging.info(f"Uncompressed size of the weights: {approx_size:.1f}MB")
